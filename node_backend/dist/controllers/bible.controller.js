...
export const getBibles = async (_req, res) => {
    try {
        ...
        const bibles = json.data
            .filter((b) => preferredBibleIds.includes(b.id))
            .map((b) => ({
            id: b.id,
...
export const getChapterContent = async (req, res) => {
    ...
    try {
        const response = await fetch(`${BASE_URL}/bibles/${bibleId}/chapters/${chapterId}?content-type=${contentType}&include-notes=false&include-titles=true&include-chapter-numbers=false&include-verse-numbers=true&include-verse-spans=false`, { headers });
        if (!response.ok) {
            throw new Error(`API responded with ${response.status}: ${response.statusText}`);
        }
        const json = await response.json();
...
export const getVerse = async (req, res) => {
    ...
    try {
        const response = await fetch(`${BASE_URL}/bibles/${bibleId}/verses/${verseId}?content-type=${contentType}&include-notes=false&include-titles=false&include-chapter-numbers=false&include-verse-numbers=true&include-verse-spans=false`, { headers });
        if (!response.ok) {
            throw new Error(`API responded with ${response.status}: ${response.statusText}`);
        }
        const json = await response.json();
...
export const getBiblePassage = async (req, res) => {
    ...
    try {
        const response = await fetch(`${BASE_URL}/bibles/${bibleId}/passages/${passageId}?content-type=${contentType}&include-notes=false&include-titles=true&include-chapter-numbers=false&include-verse-numbers=true&include-verse-spans=false&parallel-passages=false`, { headers });
        if (!response.ok) {
            throw new Error(`API responded with ${response.status}: ${response.statusText}`);
        }
        const json = await response.json();
...
export const searchBible = async (req, res) => {
    ...
    try {
        const response = await fetch(`${BASE_URL}/bibles/${bibleId}/search?query=${encodeURIComponent(query)}&limit=${limit}&offset=${offset}&sort=relevance&range=`, { headers });
        if (!response.ok) {
            throw new Error(`API responded with ${response.status}: ${response.statusText}`);
        }
        const json = await response.json();
